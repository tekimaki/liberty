...
{if $gBitUser->hasPermission('p_liberty_attach_attachments') }
	...
	{forminput}
		{if $gBitSystem->getConfig("liberty_attachment_style") == "multiple"}
			...
			<input class="fileUpload" type="file" name="{$upload_name|default:'uploads[]'}" size="40" id="uploads" />
		...
		<input class="fileUpload" type="file" name="{$upload_name|default:'uploads[]'}" size="40" id="upload_{$form_id}" onchange="javascript:LibertyAttachment.uploader(this, '{$smarty.const.LIBERTY_PKG_URL}attachment_uploader.php','{tr}Please wait for the current upload to finish.{/tr}', 'liberty_upload_frame_{$form_id}', this.form.id);" />
		...
		{else}
			<input class="fileUpload" type="file" name="{$upload_name|default:'uploads[]'}" size="40" />
			{formhelp note='After selecting the file you want to upload, please return to the edit area and click the save button.'}
		{/if}
	{/forminput}
{/if}
